...
class FontManager:
    ...
    def get_font(self, font_name: str, size: int) -> Union[FreeTypeFont, BitmapFontAdapter]:
        ...
        if font_name.lower() in ["tiny64_font", "clock64_font"]:
            ...
            # Create a new adapter with the specific font type
            bitmap_adapter = BitmapFontAdapter(size, font_type=font_name.lower())
    ...
    def list_available_fonts(self) -> List[str]:
        ...
        # Add our bitmap fonts to the list
        fonts = list(sorted(self._font_paths.keys()))
        fonts.extend(["tiny64_font", "clock64_font"])
